...
public class BitsAllocator {
	 ...
	 * @return
	 */
	public long[] resolve(long uid) {
		long totalBits = BitsAllocator.TOTAL_BITS;
		// parse UID
		long sequence = (uid << (signBits + timestampBits + workerIdBits)) >>> (totalBits - sequenceBits);
		long workerId = (uid << (signBits + timestampBits)) >>> (totalBits - workerIdBits);
		long deltaSeconds = (uid << signBits) >>> (totalBits - timestampBits);
		return new long[] { deltaSeconds, workerId, sequence };
	}
